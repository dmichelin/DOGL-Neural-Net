...
def get_weather_predictions():
    ...
    if weather_predictions is None:
        ...
        useful_predictions = list(map(lambda entry: [entry['rain']['3h'] if '3h' in entry['rain'] else 0, convert_kelvin_to_fahrenheit(entry['main']['temp_max']), convert_epoch_day_to_excel_day(entry['dt']),get_day(entry['dt'])],full_predictions))
